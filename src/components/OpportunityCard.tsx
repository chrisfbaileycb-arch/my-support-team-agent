...
const OpportunityCard: React.FC<Props> = ({ opp, agentId, accent, accentHex, glow }) => {
  ...
  const prov = opp.provenance;
  const isVerified = Boolean(opp.is_verified);
          ...
          <div className="min-w-0 flex-1">
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-x-2.5 gap-y-1">
                  ...
                  {prov && (
                    ...
                    >
                      {prov === 'LIVE_SOURCE' && <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />}
                      {prov.replace('_', ' ')}
                    </span>
                  )}
                  {isVerified && (
                    <span
                      title="Directly verified through real external source provider"
                      className="inline-flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-1.5 py-0.5 text-[9px] font-bold text-emerald-700"
                    >
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-slate-50 px-2.5 py-1.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-100"
              >
